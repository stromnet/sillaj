...
class Project {
    ...
    */         
    function getTask($intProjectId, $booAll = false) {
        ...
        if ($intProjectId != 0) {
            $strWhere = "AND (sillaj_project_intProjectId = $intProjectId)";
        }
        else {
            $strWhere = '';
        }
        ...
        if ($booAll) { 
            $arrTask = $db->getAssoc($strQuery);
        }
        else {
            $arrTask = $db->getCol($strQuery);
        }
    ...
    */
    function getEvent($intProjectId) {
        global $db;               
        ...
        $arrEvent = $db->getAll("
          ...
          ORDER BY datEvent DESC, timStart DESC, timEnd DESC, strTask
        ");        
    ...
    */
    function checkInput() {
        ...
        if (!empty($_POST['cbxShare']) && ($_POST['cbxShare'] == 'on')) {
            $booShare = '1';
        }
        else {
            $booShare = '0';
    ...
    function add() {
        ...
        $arrProject = $db->query("
          INSERT INTO sillaj_project
            (intProjectId, sillaj_user_strUserId, strProject, booDisplay, strRem, booShare)
          VALUES (
            NULL,
            '". $_SESSION['strUserId'] ."',
            '". $arrInput['strProject'] ."',
...
class Task {
    ...
    */  
    function get($intTaskId = '', $booAll = false) {
        ...
        $strTask = 'strTask';
        if ($intTaskId !== '') { 
            $strWhere = 'AND (intTaskId = ' . $intTaskId .')';
        }
    ...
    */
    function getEvent($intTaskId) {
        ...
        $arrEvent = $db->getAll("
          ...
          ORDER BY datEvent DESC, timStart DESC, timEnd DESC, strProject
        ");
    ...
    function add() {
        ...
        $arrTask = $db->query("
          INSERT INTO sillaj_task
            (intTaskId, sillaj_user_strUserId, strTask, booDisplay, strRem, booShare)
          VALUES (
            NULL,
            '". $_SESSION['strUserId'] ."',
            '". $arrInput['strTask'] ."',
        ...
        if (DB::isError($arrTask)) {
            $db->query('ROLLBACK');
            raiseError($arrTask->getMessage().$arrTask->getUserInfo());
        }
...
class Event {
    ...
    */   
    ...
    /**
    * Get last x events for one user
    * 
    */   
    function getRss($strUserId) {
        global $db;
    ...
    */
    function createMaketime($input) {
        // time can be hh-mm-ss hh:mm:ss hh,mm,ss hh;mm;ss or "hh mm ss"
    ...
    */
    function checkInput() {           
        ...
        if (empty($d) && ($s > $e)) {
            raiseError(STR_BAD_TIME_INPUT_SILLAJ . " $s &gt; $e");
        }
    ...
    function getProject($strUserId, $strSort, $datStart, $datEnd) {
        ...
        $arrReport = $db->getAll("                  
          SELECT        
            ...
            CAST(TIME_FORMAT(SEC_TO_TIME(timDurationTotProject), '%H:%i') AS CHAR) AS timDurationTotMain,
    ...
    function getTask($strUserId, $strSort, $datStart, $datEnd) {
        ...
        $arrReport = $db->getAll("                  
          SELECT        
            ...
            CAST(TIME_FORMAT(SEC_TO_TIME(timDurationTotTask), '%H:%i') AS CHAR) AS timDurationTotMain,
        ...
        if (DB::isError($arrReport)) {
            raiseError($arrReport->getMessage().$arrReport->getUserInfo());
        }          
...
class SmartySillaj extends Smarty {
    ...
    function setTemplate($template) {
        ...
        if ($template && file_exists(FN_ROOT_DIR_SILLAJ .'templates/'. $template .'/')) {
            ...
            $this->cache_dir    = FN_ROOT_DIR_SILLAJ .'cache/';
            return 1;
        } 
        else {
            return 0;
    ...
    function add() {
        ...
        $arrUser = $db->query("
          INSERT INTO sillaj_user
            (strUserId, strName, strFirstname, strEmail, strPassword, booActive, booAllowOther, booUseShare, strLanguage, strTemplate)
          VALUES (
            '". $_POST['strUserId'] ."',
            '". $arrInput['strName'] ."',
    ...
    */
    function checkEmail($strEmail) {
        ...
        if (!preg_match("/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)+$/", $strEmail)) {
            raiseError(STR_MAIL_INVALID_ADDRESS_SILLAJ .' : '. $strEmail);
        }
        ...
        if ($intRes == 0) {
            raiseError(STR_MAIL_ADDRESS_NOT_FOUND_SILLAJ .' : '. $_POST['strEmail']);
        }
    }
...
/******************************************************************************/
/**
* Get the last id inserted in the database ; we don't use DB::lastId() (?)
...
class Sillaj {
    ...
    */
    function getLanguage() {
        return $this->listDir(FN_ROOT_DIR_SILLAJ . 'lang/');
    }
