...
<form action="{$smarty.server.PHP_SELF}" id="frmGanttNav" method="get" class="noprint">
  <div>
    <a href="{$smarty.server.PHP_SELF}?{$strMain}={$intObjId}&amp;datEndGantt={$datPrev}&amp;intSpan={$intSpan}" title="{#aPrevGantt#}">{#aPrevGantt#} &lt;</a>  
    ...
    <a href="{$smarty.server.PHP_SELF}?{$strMain}={$intObjId}&amp;datEndGantt={$datNext}&amp;intSpan={$intSpan}" title="{#aNextGantt#}">&gt; {#aNextGantt#}</a>
  </div>
</form>  
