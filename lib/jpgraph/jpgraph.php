...
// PUBLIC METHODS	
	...
	if( empty($_GET[_CSIM_DISPLAY]) ) {
		...
		$htmlwrap = $this->GetHTMLImageMap($aCSIMName)."\n".
		    '<img src="'.htmlentities(CSIMCACHE_HTTP_DIR.$baseimg).'" ismap=\"ismap\" usemap="#'.$aCSIMName.'" border='.$aBorder.' width='.$this->img->width.' height='.$this->img->height." alt=\"\" />\n"; // sillaj
		...
		echo "<img src='".htmlentities($aScriptName.$urlarg)."' ismap=\"ismap\" usemap='#".$aCSIMName.'\' border='.$aBorder.'  width='.$this->img->width.' height='.$this->img->height." alt=\"\" />\n"; // sillaj
	    }
	}
	else {
	    $this->Stroke();
